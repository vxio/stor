...
class ProductPage extends Component {
  constructor (props) {
    ...
    this.sameProductsWithDifferentColors = products[match.params.category].filter(
      product => match.params.name === product.name && match.params.brand
        === product.brand,
    )
  ...
  getProductFromUrlParams (color) {
    const product = this.sameProductsWithDifferentColors.filter(
  ...
  onSubmit = values => {
    ...
    this.props.addToCart(product, () => {
      const limitReached = productInCart && productInCart.quantity
        >= this.productLimit - 1
      this.setState({ cartNotificationText: this.purchasedNotification },
        function () {
  ...
  componentWillReceiveProps (nextProps) {
    if (this.props.match.params.color !== nextProps.match.params.color) {
      this.setState(
        {
          product: {
            ...this.getProductFromUrlParams(nextProps.match.params.color),
  ...
  render () {
    ...
    const colorOptions = this.sameProductsWithDifferentColors.map(product => (
      <StyledColor key={product.color}
                   to={pathname.replace(params.color, product.color)} replace>
    ...
    return <Grid>
      <Styled>
        <img className="image" onError={this.handleImageError} src={product.img}
             alt={product.img}/>
          ...
          <Form onSubmit={this.onSubmit}
                render={({ handleSubmit, reset, submitting, pristine, values }) => {
                  return <form onSubmit={handleSubmit}>
                    <WarningText id="size-text" className="color-size-text"
                                 warn={!selectedSize && clickedSubmit}
                    ...
                    <RadioInputs name="size" options={this.sizeOptions}
                                 customOnChange={this.handleSizeInputChange}
                                 error={!selectedSize && clickedSubmit}/>
                    <Button color={(quantityLimitReached && 'disabled')
                    || (isButtonDisabled && 'primary') || 'black'} type="submit"
                            disabled={isButtonDisabled} width="25rem">
                      {cartNotificationText || (quantityLimitReached
                        && this.limitReachedNotification) || 'Add to Cart'}
                    </Button>
                  </form>
                }}/>
...
const Color = styled.div`
  ...
  display: inline-block;
  border: ${props => props.colorCode === '#fff'
  && `1px solid ${props.theme.grey_1}`};
  width: 25px;
  height: 25px;
  border-radius: 50%;
  justify-content: center;
